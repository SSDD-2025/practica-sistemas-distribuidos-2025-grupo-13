...
public class DataBaseInitializer {
    ...
    public void init() {
        ...
        Event shoko = new Event("SHOKO", "Largas noches de música latina acompañado de buen ambiente", startClub,
                finishClub, "Sala Shoko Madrid", "club", 23, shokoImage);
        Event ohmyclub = new Event("OH MY CLUB",
                "Descubre un nuevo universo con el reservado ambiente de nustra discoteca", startClub, finishClub,
                "C/Rosario Pino 14", "club", 24, ohmyclubImage);
        Event liberata = new Event("LIBERATA", "Música de ambienta acompañada de gente dispuesta a pasarselo bien",
        ...
        Event natosywaor = new Event("NATOS Y WAOR", "", startConcert, finishConcert, "Movistar Arena", "concierto", 42,
                natosywaorImage);
        Event alsafir = new Event("AL SAFIR", "", startConcert, finishConcert, "Las Ventas", "concierto", 29,
                alsafirImage);
        Event pekeno = new Event("ILL PEKEÑO & ERGO PRO", "", startConcert, finishConcert, "Sala Changó", "concierto",
                15, pekeImage);
        ...
        for (int i = 0; i < 10; i++) {
            ticketRepository
                    .save(new Ticket(shoko.getTitle(), shoko.getPrecio(), shoko.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(
                    new Ticket(ohmyclub.getTitle(), ohmyclub.getPrecio(), ohmyclub.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(
                    new Ticket(liberata.getTitle(), liberata.getPrecio(), liberata.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(
                    new Ticket(madcool.getTitle(), madcool.getPrecio(), madcool.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(new Ticket(blackworks.getTitle(), blackworks.getPrecio(), blackworks.getTimeFinish(),
                    TicketStatus.OPEN));
            ticketRepository.save(new Ticket(madridSalvaje.getTitle(), madridSalvaje.getPrecio(),
                    madridSalvaje.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(
                    new Ticket(rioBabel.getTitle(), rioBabel.getPrecio(), rioBabel.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository.save(new Ticket(natosywaor.getTitle(), natosywaor.getPrecio(), natosywaor.getTimeFinish(),
                    TicketStatus.OPEN));
            ticketRepository.save(
                    new Ticket(alsafir.getTitle(), alsafir.getPrecio(), alsafir.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository
                    .save(new Ticket(pekeno.getTitle(), pekeno.getPrecio(), pekeno.getTimeFinish(), TicketStatus.OPEN));
            ticketRepository
                    .save(new Ticket(cruzzi.getTitle(), cruzzi.getPrecio(), cruzzi.getTimeFinish(), TicketStatus.OPEN));
        ...
        Ticket tcktejmpl2 = new Ticket(natosywaor.getTitle(), natosywaor.getPrecio(), natosywaor.getTimeFinish(),
                TicketStatus.OPEN);
